...
# Wrapper function for Test Tool's ttool command.
ttool-color() {
  # Filter ttool output using sed
  ttool $@ | sed -e "s/\(\[INFO\]\ \-.*\)/${TEXT_BLUE}${BOLD}\1/g" \
...
# Override the mvn command with the colorized one. (not yet implemented accordingly :P)
#alias ttool="ttool-color"
