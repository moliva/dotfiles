...
mvn-color()
{
  ...
  mvn $@ | sed -e "s/\(\[INFO\]\ \-.*\)/\1/g" \
               ...
               -e "s/\(\SUCCESS - Test .*\)/${TEXT_GREEN}\1${RESET_FORMATTING}/g" \
               -e "s/\(\SKIPPED - Test .*\)/${TEXT_GREEN}\1${RESET_FORMATTING}/g" \
               -e "s/\(\ERROR - The test .*\)/${TEXT_RED}\1${RESET_FORMATTING}/g" \
               -e "s/\(\FAILURE - The test .*\)/${TEXT_RED}\1${RESET_FORMATTING}/g" \
	       -e "s/\(\.*---\)\(.*\)\( <<< ERROR.*\)/${TEXT_RED}\1${UNDERLINE_ON}${BOLD}\2${RESET_FORMATTING}${TEXT_RED}\3${RESET_FORMATTING}/g" \
	       -e "s/\(\.*---\)\(.*\)\( <<< FAILED.*\)/${TEXT_RED}\1${UNDERLINE_ON}${BOLD}\2${RESET_FORMATTING}${TEXT_RED}\3${RESET_FORMATTING}/g" \
	       -e "s/Tests run: \([^,]*\), Failures: \(0\), Errors: \(0\), Skipped: \(0\)\(.*\)/${TEXT_GREEN}Tests run: \1, Failures: \2, Errors: \3, Skipped: \4\5${RESET_FORMATTING}/g" \
               -e "s/Tests run: \([^,]*\), Failures: \(0\), Errors: \(0\), Skipped: \([123456789]\d*\)\(.*\)/${TEXT_YELLOW}Tests run: \1, Failures: \2, Errors: \3, Skipped: \4\5${RESET_FORMATTING}/g" \
               -e "s/Tests run: \([^,]*\), Failures: \([123456789]\d*\), Errors: \([123456789]\d*\), Skipped: \([^,]*\)\(.*\)/${TEXT_RED}Tests run: \1, Failures: \2, Errors: \3, Skipped: \4\5${RESET_FORMATTING}/g" \
               -e "s/\(\s*\)>> \(.*\) test result: Errors: \(0\), Failures:\(0\)/${TEXT_GREEN}\1>> ${UNDERLINE_ON}${BOLD}\2${RESET_FORMATTING}${TEXT_GREEN} test result: Errors: \3, Failures: \4${RESET_FORMATTING}/g" \
               -e "s/\(\s*\)>> \(.*\) test result: Errors: \([123456789]\d*\), Failures:\([^,]*\)/${TEXT_RED}\1>> ${UNDERLINE_ON}${BOLD}\2${RESET_FORMATTING}${TEXT_RED} test result: Errors: \3, Failures: \4${RESET_FORMATTING}/g" \
               -e "s/\(\s*\)>> \(.*\) test result: Errors: \([^,]*\), Failures:\([123456789]\d*\)/${TEXT_RED}\1>> ${UNDERLINE_ON}${BOLD}\2${RESET_FORMATTING}${TEXT_RED} test result: Errors: \3, Failures: \4${RESET_FORMATTING}/g" \
               -e "s/Number of tests run: \([^,]*\) - Failed: \(0\) - Errors: \(0\) - Skipped: \(0\)/${TEXT_GREEN}Number of tests run: \1 - Failed: \2 - Errors: \3 - Skipped: \4${RESET_FORMATTING}/g" \
               -e "s/Number of tests run: \([^,]*\) - Failed: \(0\) - Errors: \(0\) - Skipped: \([123456789]\d*\)/${TEXT_YELLOW}Number of tests run: \1 - Failed: \2 - Errors: \3 - Skipped: \4${RESET_FORMATTING}/g" \
               -e "s/Number of tests run: \([^,]*\) - Failed: \([^,]*\) - Errors: \([123456789]\d*\) - Skipped: \([^,]*\)/${TEXT_RED}Number of tests run: \1 - Failed: \2 - Errors: \3 - Skipped: \4${RESET_FORMATTING}/g" \
               -e "s/Number of tests run: \([^,]*\) - Failed: \([123456789]\d*\) - Errors: \([^,]*\) - Skipped: \([^,]*\)/${TEXT_RED}Number of tests run: \1 - Failed: \2 - Errors: \3 - Skipped: \4${RESET_FORMATTING}/g" 
